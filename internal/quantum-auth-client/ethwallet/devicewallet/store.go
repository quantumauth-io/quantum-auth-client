...
var ErrDeviceWalletExists = errors.New("devicewallet: wallet already exists")

func NewStore(sealer tpmdevice.Sealer) (*Store, error) {
	if sealer == nil {
		return nil, errors.New("devicewallet: sealer is required")
	}
...
func (s *Store) Load(ctx context.Context) (*DeviceWallet, error) {
	wf, err := s.readFile()
	if err == nil {
...
func (s *Store) Create(ctx context.Context) (*DeviceWallet, error) {
	ok, err := s.Exists()
	if err != nil {
		return nil, err
	}
	if ok {
		return nil, ErrDeviceWalletExists
	}
...
func (s *Store) decryptPrivKeyBytes(ctx context.Context, wf deviceWalletFile) ([]byte, error) {
	...
	dek, err := s.Sealer.Unseal(ctx, constants.SealerLabel, sealed)
	if err != nil {
	...
	plainJSON, err := aead.Open(nil, nonce, ct, []byte(constants.PayloadAAD))
	if err != nil {
...
func (s *Store) createNew(ctx context.Context) (*DeviceWallet, deviceWalletFile, error) {
	...
	// 3) Seal DEK with TPM
	sealed, err := s.Sealer.Seal(ctx, constants.SealerLabel, dek)
	if err != nil {
	...
	ct := aead.Seal(nil, nonce, plainJSON, []byte(constants.PayloadAAD))
